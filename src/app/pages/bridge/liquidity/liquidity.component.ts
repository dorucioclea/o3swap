...
import { ApiService, CommonService, MetaMaskWalletApiService, SwapService } from '@core';
import { Observable } from 'rxjs';
import { Store } from '@ngrx/store';
import { SwapStateType } from 'src/app/_lib/swap';
import { SWAP_CONTRACT_CHAIN_ID, METAMASK_CHAIN, METAMASK_CHAIN_ID, BRIDGE_SLIPVALUE, CHAIN_TOKENS, Token } from '@lib';
...
export class LiquidityComponent implements OnInit, OnDestroy {
  ...
  async changeOutAmount(token: Token, index: number): Promise<void> {
    if (!new BigNumber(this.removeLiquidityInputAmount[index]).isNaN()) {
      this.payAmount = await this.apiService.getPoolInGivenSingleOut(token, this.removeLiquidityInputAmount[index]);
    } else {
      this.payAmount = '';
    }
  }
  ...
  async maxRemoveLiquidityInput(index: number): Promise<void> {
    if (!new BigNumber(this.LPToken.amount).isNaN()) {
      ...
      this.removeLiquidityInputAmount[index] =
        await this.apiService.getSingleOutGivenPoolIn(this.addLiquidityTokens[index], this.payAmount);
    }
  }
  ...
  async depost(token: Token, index: number): Promise<void> {
    ...
    const amountOut = new BigNumber(this.receiveAmount).shiftedBy(18).dp(0).toFixed();
    const minAmountOut = this.swapService.getAmountOutMinWithAmountOut(
      amountOut,
      BRIDGE_SLIPVALUE
    );
  ...
  async withdrawal(token: Token, index: number): Promise<void> {
    ...
    const amountOut = new BigNumber(this.removeLiquidityInputAmount[index]).shiftedBy(token.decimals).dp(0).toFixed();
    const minAmountOut = this.swapService.getAmountOutMinWithAmountOut(
      amountOut,
      BRIDGE_SLIPVALUE
    );
  ...
  private getLPBalance(): void {
    ...
    const token = this.LPTokens.find(item => item.chain === this.currentChain);
